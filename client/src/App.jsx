...
function detectIntent(text, lang) {
  ...
  if (numMatch) {
    ...
    if (train) {
      const delayText = train.delay > 0 ? ` Running ${train.delay} min late.` : ` On time.`;
      const delayHindi = train.delay > 0 ? ` ${train.delay} मिनट देरी से चल रही है।` : ` समय पर है।`;
      return {
        intent: "train_status", train,
        response: lang === "hi"
          ? `ट्रेन ${train.number} – ${train.nameHindi} | प्लेटफॉर्म ${train.platform}।${delayHindi}`
          : `Train ${train.number} – ${train.name} | Platform ${train.platform}.${delayText}`,
        highlight: "train_status"
      };
    }
    return { intent:"not_found", response: lang==="hi" ? `ट्रेन ${numMatch[1]} नहीं मिली।` : `Train ${numMatch[1]} not found.`, highlight:null };
  }
  ...
  if (pfMatch) {
    ...
    if(pf){
      const pfTrains = TRAINS.filter(t=>t.platform===n).map(t=>t.name).join(", ");
      return { intent:"platform", platform:pf, response: lang==="hi" ? `प्लेटफॉर्म ${n} पर: ${pfTrains||"कोई ट्रेन नहीं"}` : `Platform ${n}: ${pfTrains||"No trains currently"}`, highlight:"map" };
    }
  }
  if (/crowd|congestion|भीड़/.test(lower)) {
    const sorted = [...PLATFORMS].sort((a,b)=>b.crowdLevel-a.crowdLevel);
    return { intent:"crowd", response: lang==="hi" ? `प्लेटफॉर्म ${sorted[0].id} पर सबसे अधिक भीड़ (${sorted[0].crowdLevel}%)।` : `Platform ${sorted[0].id} is most crowded (${sorted[0].crowdLevel}%).`, highlight:"crowd" };
  }
  if (/food|eat|खाना|restaurant/.test(lower)) return { intent:"facility", response: lang==="hi" ? "फूड कोर्ट प्लेटफॉर्म 5-6 के पास है।" : "Food Court is near platforms 5-6.", highlight:"map" };
  if (/ticket|टिकट/.test(lower)) return { intent:"facility", response: lang==="hi" ? "टिकट काउंटर मुख्य प्रवेश के पास है।" : "Ticket counters are near the main entrance.", highlight:"map" };
  if (/wait|rest|प्रतीक्षा/.test(lower)) return { intent:"facility", response: lang==="hi" ? "प्रतीक्षा कक्ष प्लेटफॉर्म 1, 2 और 4 के पास है।" : "Waiting rooms near Platforms 1, 2, and 4.", highlight:"map" };
  return { intent:"unknown", response: lang==="hi" ? "माफ़ करें, मैं समझ नहीं पाया। ट्रेन नंबर या सुविधा पूछें।" : "Sorry, I didn't understand. Please ask about a train number or facility.", highlight:null };
}
